...
dotnet publish $project -p:PublishProfile=win-x64 -c $Configuration -o $home
if ($LASTEXITCODE -ne 0) {
    throw "dotnet publish falló."
}

Copy-Item -Recurse -Force $home $tech
...
Compress-Archive -Path (Join-Path $home "*") -DestinationPath $homeZip
